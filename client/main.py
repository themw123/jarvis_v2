...
def main():
    ...
    listen_keyboard_interupt = threading.Thread(
        target=Lifecircle.listen_to_interupt_keyboard, args=(recorder, config))
    ...
    while True:
        ...
        stt_text = stt.request_backend_stt(
            audio=audio, rate=recorder.rate, sample_width=recorder.p.get_sample_size(recorder.format))
        print_user(stt_text)
        print_assistant()
    ...
    while True:
        try:
            response = requests.post(
                url, headers=headers, data=json.dumps(config))
...
def print_user(stt_text):
    print("\n" + colorama.Fore.YELLOW + "(" +
          config["chat"]["your_name"] + "):", stt_text + colorama.Style.RESET_ALL)
    pass


def print_assistant():
    print("\n" + colorama.Fore.GREEN +
          "("+config["chat"]["role_name"]+"): ", end="")
    pass
