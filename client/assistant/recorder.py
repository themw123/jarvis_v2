...
class Recorder:
    ...
    def listen_on_keyboard(self):
        while True:
            listener1 = keyboard.Listener(
                ...
                on_release=lambda key: self.__on_release_ctrl_space(
                    key, "listener1")
            )
    ...
    def listen_on_voice(self):
        ...
        with sr.Microphone() as source:
            ...
            recognizerSpokenText.adjust_for_ambient_noise(
                source, duration=self.config["recorder"]["ambient_record_time"])
            Player.play_initial()
            print("\n- waiting for you...")
            while True:
                try:
                    self.websocket.send_bytes(
                        self.stream_wakeword.read(self.chunk))
                    ...
                    while True:
                        try:
                            ...
                            audio_bytes = audio_data.get_wav_data(
                                convert_rate=self.rate, convert_width=self.p.get_sample_size(self.format))
