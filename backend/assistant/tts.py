...
class Tts:
    ...
    def __init__(self, server_config, client_config):
        ...
        try:
            ...
            temp_audio_path = os.path.abspath(
                os.path.join(current_dir, "..", "temp_audio"))
    ...
    def tts_wrapper(self, brain_text):
        ...
        if self.client_config["tts"]["active"] == "google":
            ...
            raise Exception(
                self.client_config["tts"]["active"] + ": This tts api type does not exist")

    def __tts_google(self, brain_text):
        stream = gTTS(
            text=brain_text, lang=self.server_config["tts"]["google"]["language"], slow=False).stream()
        for bytes in stream:
    ...
    def __tts__xtts_init(self):
        ...
        server_config.load_json(
            self.server_config["tts"]["xtts"]["location"] + "/config.json")
        self.xtts_model = Xtts.init_from_config(server_config)
        self.xtts_model.load_checkpoint(
            server_config, checkpoint_dir=self.server_config["tts"]["xtts"]["location"], use_deepspeed=True)
    ...
    def __tts_piper_init(self):
        ...
        voice_path = self.server_config["tts"]["piper"]["location"] + \
            "/voices/"+self.client_config["tts"]["piper_voice"]

        files = os.listdir(voice_path)
        self.piper_model_path = next(
            (os.path.join(voice_path, f) for f in files if f.endswith('.onnx')), None)
        self.piper_json_path = next(
            (os.path.join(voice_path, f) for f in files if f.endswith('.json')), None)
    ...
    def __tts_piper_gen(self, brain_text):
        ...
        output_file = tempfile.NamedTemporaryFile(
            delete=False, dir=temp_audio_path, suffix=".wav")
        ...
        self.process = subprocess.Popen(
            command, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
    ...
    def __tts_piper_stream(self, brain_text):
        ...
        voice_path = self.server_config["tts"]["piper"]["location"] + \
            "/voices/"+self.server_config["tts"]["piper"]["voice"]

        files = os.listdir(voice_path)
        model_path = next((os.path.join(voice_path, f)
                          for f in files if f.endswith('.onnx')), None)
        json_path = next((os.path.join(voice_path, f)
                         for f in files if f.endswith('.json')), None)
